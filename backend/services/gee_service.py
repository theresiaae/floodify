...
def _get_soil_moisture(point, end_date: str):
    # ERA5-Land punya jeda publikasi (bisa beberapa minggu) sehingga window
    # tetap yang pendek berisiko kosong (None). Jadi di sini kita ambil
    # citra TERBARU yang sudah tersedia sampai end_date, bukan rata-rata
    # window tetap — supaya tetap dapat nilai walau ada jeda publikasi.
    collection = (
    ...
    # ERA5-Land resolusinya kasar (~11 km/pixel). Titik di dekat garis
    # pantai/pesisir kadang jatuh di grid cell yang dianggap "laut" oleh
    # dataset ini (masked -> null), walau lokasinya nyata daratan. Buffer
    # kecil (30m) cuma nyentuh 1 pixel, jadi kalau pixel itu ter-mask,
    # hasilnya kosong. Buffer di sini dilebarkan supaya reduceRegion bisa
    # menangkap pixel tetangga yang valid dan dirata-rata (mean otomatis
    # mengabaikan pixel yang masked/null).
...
def get_gee_parameters(lat: float, lng: float, target_date=None) -> dict:
    ...
    target_date (datetime.date, opsional): tanggal acuan untuk window NDVI
    dan kelembapan tanah. elevasi & tutupan_lahan selalu statis, tidak
    terpengaruh tanggal. Kalau target_date ada di masa depan, di-clamp ke
    hari ini — GEE cuma punya citra satelit observasi, tidak ada citra
    untuk tanggal yang belum terjadi.
    """
    _initialize()
    ...
    ndvi_end = _date_str(anchor_dt + timedelta(days=1))  # filterDate end is exclusive
